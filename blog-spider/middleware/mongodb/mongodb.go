...
var (
	MongoLogChannel = make(chan MongoLog, 300)
	MonCli          *mongo.Client
)
...
func InitMongo() {
	MonCli, _ = CreatePool(
		fmt.Sprintf("mongodb://%s:%s@%s:%d", config.Conf.Mongo.Username, config.Conf.Mongo.Password,
			config.Conf.Mongo.Host, config.Conf.Mongo.Port,
		), 10,
	)
}
...
// NewConnection 初始化db连接
func NewConnection(database, collection string) *Collection {
	mgo := new(Collection)
	mgo.database = database
	mgo.collection = collection
	mgo.client = MonCli.Database(mgo.database).Collection(mgo.collection)
	return mgo
}
...
// FindOne 查找单个文档
func (m *Collection) FindOne(filter bson.D) (interface{}, error) {
	var result interface{}
	err := m.client.FindOne(context.Background(), filter).Decode(&result)
	if err != nil {
...
// FindOneByFilter 查找单个文档, sort 等于1表示 返回最旧的，sort 等于-1 表示返回最新的
/*
...
E：D里面的一个元素
*/
func (m *Collection) FindOneByFilter(filter bson.D, sort, projection bson.M) (bson.M, error) {
...
// CollectionCount 快速统计集合中所有文档的数量
